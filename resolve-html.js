...
let { startYear, lunarYear, lunarMon, nmCN, lunarCN, divisionsTr } = require('./config');
    ...
    getHtml(year).then(function (data) {
        ...
        arr.map(function (item) {
            if (item && item.replace(/\s/g, '')) {
                let info = item.split(' '), temp = true, solar = info[ 0 ].split(/[^0-9]/), lunar = info[ 1 ],
                    divisions = divisionsTr.indexOf(info[ 3 ]) + 1;

                //判断是否带月，带月表示当前月第一天
                if (lunar.indexOf('月') > 0) {
                    //解析旧历月份
                    lunarMon = countOldToNum(lunar.replace('月', ''));
                    //月份为一时，说明是下一年，年份加一
                    //防止润一月时，lunarYear重复加
                    if (lunarMon === 1 && temp) {
                ...
                lunar = [ lunarYear, lunarMon, countOldToNum(lunar) ].join('-');
                result[ solar ] = { s: solar, l: lunar };
                if (divisions) result[ solar ].d = divisions;
            }
        });
        writeFile(result, year, function (nextYear) {
            resolveHtml(nextYear)
        });
    });
}
